...
function buildMarks( $marks )
{
	...
	for ($i = 0; $i < count($marks); $i++)
	{
		...
		/* Title */
		$marksHTML = $marksHTML . "<div class=\"blogmark\">\n<h3><a href=\"" . $marks[$i]["url"] . "\">" . $marks[$i]["title"] . "</a></h3>\n";
		/* Rant itself */
...
function buildCondensedMarks( $marks )
{
	...
	for ($i = 0; $i < count($marks); $i++)
	{
		$marksHTML .= "<div id=\"url" . $marks[$i]["id"] . "\"><div class=\"blogmark\">\n";
		/* Title */
		$marksHTML .= "<h3><a href=\"" . $marks[$i]["url"] . "\">" . $marks[$i]["title"] . "</a></h3>\n";
		...
		$marksHTML .= " | <a href=\"" . markUrl($marks[$i]) . "\">" . getMonthDate($marks[$i]["date"]) . ", " . getTime($marks[$i]["date"]) . "</a>";
		$marksHTML .= " ]</div>\n";
		$marksHTML .= "</div></div>\n\n";
	}
	return $marksHTML;
...
function buildSimpleMarks( $marks )
{
	...
	for ($i = 0; $i < count($marks); $i++)
	{
		//$marksHTML .= "\t\t\t<li><a href=\"" . markUrl($marks[$i]) . "\">" . $marks[$i]["title"] . "</a></li>\n";
		$marksHTML .= "\t\t\t<li><a href=\"" . $marks[$i]["url"] . "\">" . $marks[$i]["title"] . "</a> <span class=\"note\">[<a href=\"" . markUrl($marks[$i]) . "\">more</a>]</span></li>\n";
	}
	$marksHTML .= "\t\t</ul>\n";
	return $marksHTML;
}
...
function buildComments( $comments )
{
	...
	for ($i = 0; $i < count($comments); $i++)
	{
		...
		if ($comments[$i]["url"] != "")
		{
			$url = "<a href=\"" . $comments[$i]["url"] . "\">" . $comments[$i]["name"] . "</a>";
		}
		...
		$commentsHTML .= ($i + 1) . ".&nbsp;</span><span class=\"comment_datestamp\">Posted at " . $comments[$i]["date"] . "</span>&nbsp;<span class=\"comment_name\">by " . $url . "</span>";
